...
const getDataById = async(req, res)=>{
    ...
    if(!user){
        return res.status(404).json({status: "Failed", message: "User with given Id does not exist"
    }
        )
    }
    res.json({
...
const deleteData = async(req, res)=>{
    ...
    if(!user){
        return res.status(404).json({'status': 'Failed', 'message':'User with Id not found'})
    }
    res.json({
        status: 'success',
